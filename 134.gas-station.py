...
# @lc code=start
class Solution:
    def judge(self,gas,pos):
        result = 0
        for i in range(pos,len(gas)):
            result += gas[i]
            if(result < 0):return 0,i+1
        for i in range(pos):
            result += gas[i]
            if(result < 0):return 0,i+1
        return 1,pos
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        new_gas = [gas[i] - cost[i] for i in range(len(gas))]
        judge,pos = self.judge(new_gas,0)
        max_pos = 0
        while(judge == 0):
            print(pos,judge,max_pos)
            if(max_pos >= pos):return -1
            else : max_pos = pos
            judge,pos = self.judge(new_gas,pos)
        return pos
# @lc code=end
